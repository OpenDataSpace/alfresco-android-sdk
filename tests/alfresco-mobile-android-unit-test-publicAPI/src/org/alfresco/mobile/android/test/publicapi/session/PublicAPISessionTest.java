...
public class PublicAPISessionTest extends SessionTest
{
    ...
    public void dispatchSession()
    {
        try
        {
            ...
            RepositorySession session = RepositorySession
                    .connect(HOSTNAME + "/alfresco/service/cmis", ALFRESCO_CMIS_USER, ALFRESCO_CMIS_PASSWORD);
            ...
            Assert.assertTrue(session.getServiceRegistry()
                    .getDocumentFolderService() instanceof OnPremiseDocumentFolderServiceImpl);
            Assert.assertTrue(
                    session.getServiceRegistry().getWorkflowService() instanceof OnPremiseWorkflowServiceImpl);
            ...
            session = RepositorySession
                    .connect(HOSTNAME + "/alfresco/cmisatom", ALFRESCO_CMIS_USER, ALFRESCO_CMIS_PASSWORD, settings);
            ...
            session = RepositorySession
                    .connect(HOSTNAME + "/alfresco/api/-default-/public/cmis/versions/1.0/atom/", ALFRESCO_CMIS_USER,
            ...
            Assert.assertTrue(session.getRepositoryInfo().getMajorVersion() >= OnPremiseConstant.ALFRESCO_VERSION_4);
            Assert.assertTrue(session.getServiceRegistry()
                    .getDocumentFolderService() instanceof PublicAPIDocumentFolderServiceImpl);
            Assert.assertTrue(
                    session.getServiceRegistry().getWorkflowService() instanceof PublicAPIWorkflowServiceImpl);
