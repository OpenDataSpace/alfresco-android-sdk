...
public abstract class AbstractAlfrescoSessionImpl implements AlfrescoSession, Parcelable
{
    ...
    public static final String DEFAULT_CACHE_FOLDER_PATH =
            Environment.getExternalStorageDirectory() + "/Android/data/org.alfresco.mobile.android.sdk/cache";
    ...
    protected final Map<String, String> sessionParameters = new HashMap<String, String>();
    ...
    {
        return userParameters != null && userParameters.containsKey(key);
    }
    ...
    private void createCloudCmisSettings()
    {
        ...
        if (!sessionParameters.containsKey(BINDING_URL))
        {
            sessionParameters.put(SessionParameter.ATOMPUB_URL,
                    ((String) getParameter(BASE_URL)).concat(CloudUrlRegistry.BINDING_NETWORK_CMISATOM)
                            .replace(CloudUrlRegistry.VARIABLE_NETWORKID,
                                    (String) getParameter(CloudSession.CLOUD_NETWORK_ID)));
        }
        ...
        try
        {
            if (param.get(SessionParameter.REPOSITORY_ID) != null)
            {
                return ((SessionFactoryImpl) sessionFactory)
                        .createSession(param, null, new PassthruAuthenticationProviderImpl(authenticator), null);
            }
            else
            {
                return ((SessionFactoryImpl) sessionFactory)
