...
public final class OnPremiseUrlRegistry
{
    ...
    {
        return session.getBaseUrl().concat(PREFIX_SERVICE)
                .concat(URL_USER_PREFERENCES.replace(VARIABLE_USER, getEncodingPersonIdentifier(username))
                        .replace(VARIABLE_PREFERENCE, preferenceFilter));
    }
    ...
    {
        return session.getBaseUrl().concat(PREFIX_SERVICE)
                .concat(URL_LEAVE_SITE.replace(VARIABLE_SHORTNAME, siteShortName)
                        .replace(VARIABLE_USER, getEncodingPersonIdentifier(username)));
    }
    ...
    {
        return session.getBaseUrl().concat(PREFIX_SERVICE)
                .concat(URL_CANCEL_JOIN_SITE_REQUEST.replace(VARIABLE_SHORTNAME, siteIdentifier)
                        .replace(VARIABLE_INVITEID, getEncodingPersonIdentifier(inviteId)));
    }
    ...
    {
        return session.getBaseUrl().concat(PREFIX_SERVICE)
                .concat(URL_MEMBEROF.replace(VARIABLE_SHORTNAME, siteIdentifier)
                        .replace(VARIABLE_USER, getEncodingPersonIdentifier(inviteId)));
    }
    ...
    {
        return session.getBaseUrl().concat(PREFIX_SERVICE)
                .concat(URL_ALL_TAGS.replace(VARIABLE_STORE_TYPE, NodeRefUtils.PROTOCOL_WORKSPACE)
                        .replace(VARIABLE_STORE_ID, NodeRefUtils.IDENTIFIER_SPACESSTORE));
    }
    ...
    // RENDITION
    // //////////////////////////////////////////////////////////////////////////////
    public static final String URL_THUMBNAILS =
            "api/node/{store_type}/{store_id}/{id}/content/thumbnails/{thumbnailname}";
     ...
     * @since 1.3.0
     */
    public static final String URL_PERSON_GUID =
            "api/forms/picker/authority/children?selectableType=cm:person&searchTerm={username}&size=1";
    ...
    {
        return session.getBaseUrl().concat(PREFIX_SERVICE).concat(URL_TASK_FOR_PROCESS)
                .replace(VARIABLE_PROCESSID, processId);
    }
    ...
    {
        return session.getBaseUrl().concat(PREFIX_SERVICE).concat(URL_PROCESS_DIAGRAM)
                .replace(VARIABLE_PROCESSID, processId);
    }
    ...
    {
        return session.getBaseUrl().concat(PREFIX_SERVICE).concat(URL_FORM_PROCESS)
                .replace(VARIABLE_PROCESSKEY, processKey);
    }
    ...
    {
        return session.getBaseUrl().concat(PREFIX_SERVICE).concat(URL_PERSON_GUID)
                .replace(VARIABLE_USERNAME, getEncodingPersonIdentifier(username));
    }
